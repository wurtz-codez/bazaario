...
// Handle website preview requests
router.get('/:slug', async (req, res) => {
  try {
    const { slug } = req.params;
    
    // Find website with the provided slug/subdomain
    const website = await Website.findOne({ 
      domain: new RegExp(`^${slug}`, 'i') // Match beginning of domain field
    }).populate('template');
      ...
      return res.status(404).send(`
        <html>
          ...
          <body style="font-family: Arial, sans-serif; padding: 2rem; text-align: center;">
            ...
            <p><a href="/">Return to Bazaario</a></p>
          </body>
        </html>
      `);
    }
    
    // For local development environment, we'll show the preview regardless of publish status
    const isDevelopment = process.env.NODE_ENV === 'development';
    
    // Only check published status in production
    if (!isDevelopment && !website.isPublished) {
      return res.status(200).send(`
        <html>
          <head>
            <title>${website.name} - Preview Mode</title>
            <style>
              body {
                font-family: "${website.settings?.fonts?.body || 'Arial'}", sans-serif;
                margin: 0;
                padding: 0;
                background-color: ${website.settings?.colors?.background || '#ffffff'};
                color: ${website.settings?.colors?.text || '#333333'};
              }
              .banner {
              ...
              .btn {
                background-color: ${website.settings?.colors?.secondary || '#f50057'};
                color: white;
                border: none;
          ...
          <body>
            <div class="banner">
              This website is currently in draft mode and not published.
            </div>
            <div class="container">
              <h1>${website.name}</h1>
    ...
    // Try to fetch some products for this website
    const products = await Product.find({ website: website._id }).limit(6);
    
    // Generate improved preview with functional components
    res.send(`
      <!DOCTYPE html>
      <html lang="en">
        <head>
    ...
    res.status(500).send(`
      <html>
        ...
        <body style="font-family: Arial, sans-serif; padding: 2rem; text-align: center;">
          ...
          <p>There was a problem loading the website preview.</p>
          <p>Error details: ${process.env.NODE_ENV === 'development' ? error.message : 'Contact support for assistance'}</p>
          <p><a href="/">Return to Bazaario</a></p>
        </body>
      </html>
    `);
